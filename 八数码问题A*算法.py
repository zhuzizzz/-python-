...
def solvePuzzle_A(srcLayout, destLayout):
    ...
    g_dict_layouts[srcLayout] = -1  #初始化
    ...
    while g_dict_layouts[curLayout] != -1:#存入路径
        ...
        lst_steps.append(curLayout)
    lst_steps.reverse()
    return 0, lst_steps, nodes_opened, nodes_new, g_dict_layouts_deep[newLayout]


if __name__ == "__main__":
	
    srcLayout = input("请输入初始状态：(如283104765)") #测试数据输入格式
    start=time.process_time()
    destLayout = "123804765"

    retCode, lst_steps, nodes_opened, nodes_new, deep = solvePuzzle_A(srcLayout, destLayout)
    if retCode != 0:
        print("目标布局不可达")
    else:
